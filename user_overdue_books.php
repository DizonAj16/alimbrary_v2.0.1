...
if (isset($_SESSION["id"]) && !empty(trim($_SESSION["id"]))) {
    // Prepare a select statement to count overdue books
    $sql = "SELECT COUNT(*) AS total_overdue FROM borrowed_books 
            ...
            WHERE borrowed_books.user_id = ? AND borrowed_books.return_date <= CURRENT_DATE AND return_history.borrow_id IS NULL";
    ...
    if ($stmt = mysqli_prepare($conn, $sql)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_stmt_num_rows($stmt) > 0) {
                ...
                echo "<h3 class='fw-bold text-center'>" . $total_overdue . " Overdue Books</h3>";
            } else {
                echo "<h3 class='fw-bold text-center' style='font-size: 30px;'>No overdue books</h3>";
            }
        } else {
